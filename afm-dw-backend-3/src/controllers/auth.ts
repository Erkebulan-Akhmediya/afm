...
import {employee_create,employee_status, employee_education_create, employee_work_list_create, employee_crime_create, employee_language_create, employee_sport_create, employee_fine_create, employee_adgs_create} from '../db_apis/employee/employee'
...
export async function userRegister (req: express.Request, res: express.Response, next: express.NextFunction){
    ...
    try {
        ...
        await employee_status({user_id: employee[0].id})
            .catch(e => { throw `Ошибка создания статуса пользователя : ${e.message}` });
