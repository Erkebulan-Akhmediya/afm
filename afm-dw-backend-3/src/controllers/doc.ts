...
export async function pdf_summary (req: express.Request, res: express.Response, next: express.NextFunction) { 
    try {
        ...
        console.log("_____________________TRY_TO_GET_INFO_________________________")
        console.log("Employee Information : ", query_return_data.rows[0])
        var result:any = {data: {}}
        ...
        switch (bind.work_place_type_report_code) {
            case 'pdf_workplace_base': 
                ...
                console.log('WHAT THE FUCK IS A KILOMETER', query_return_data.rows[0].last_name_rus 
                + ' ' + 
                query_return_data.rows[0].first_name_rus + 
                (
                    query_return_data.rows[0].middle_name_rus ? 
                    (' ' + query_return_data.rows[0].middle_name_rus) : ''
                ));
                                
                // result.data['employee_full_name'] = declension.parse(
                //     query_return_data.rows[0].last_name_rus 
                //     + ' ' + 
                //     query_return_data.rows[0].first_name_rus + 
                //     (
                //         query_return_data.rows[0].middle_name_rus ? 
                ...
                //     )
                // )
                // .getForm(Case.Dat, false)

                result.data['employee_full_name'] = 
                    query_return_data.rows[0].last_name_rus 
                    + ' ' + 
                    query_return_data.rows[0].first_name_rus + 
                    (
                ...
                result.data['gender_id'] = query_return_data.rows[0].gender_id == 1 ? 'он' : 'она'
