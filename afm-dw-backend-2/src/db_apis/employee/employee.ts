...
import { unviversal_query } from '../universal_db';
...
export async function employee_get_db_new(bind: employee_get_binds): Promise<Employee[]> {
    let client;
    try {
        client = get_client();
      ...
      // const { rows: employeei }: any = await client.query(select);
...
export async function employee_subordinates_get_db(client: Client, id: Number, isExistLimit: Boolean, offset: number, limit: number, only_department_id: Number, only_organization_id: Number): Promise<any> {
    try {
        ...
        select 
            ...
            e.position_id
        from 
            ret_employee_subordinates(${id}) res, 
            hr.employee e 
        where 
            res.id = e.id 
