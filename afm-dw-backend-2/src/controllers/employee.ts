...
import {employee_put_db_new,employee_get_db,employee_get_all_db,employee_education_db,employee_sports_db ,employee_fines_db,employee_adgs_db,employee_put_db,employee_languages_db ,employee_subordinates_get_db,employee_worklist_db, organization_subordinates_get_db, employee_get_positions_db, employee_get_access_scud_department_list, get_organization_dep_list, employee_birthday_get_db, existEmployeeExplanatory, employee_get_db_new} from '../db_apis/employee/employee';
...
export async function employee_change_status(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    try {
        ...
        await employee_put_db_new(client, {
            id: req.params.id,
        ...
        if (client) {
            try {
                ...
                log.error(
                    'error closing connection in controllers/employee.ts => employee_change_status'
                );
            }
        }
    }
