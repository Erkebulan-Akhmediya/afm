...
export async function lov_get_db(bind: any) {
    ...
    try {
        ...
        if (bind.education_type_id) {
            sqlWhereClause += ` ${sqlWhereClause.trim() ? ` and ` : ` where `}  t.education_type_id = ${bind.education_type_id} `
        }
        if (bind.contact_info_type_id) {
            sqlWhereClause += ` ${sqlWhereClause.trim() ? ` and ` : ` where `}  t.contact_info_type_id = ${bind.contact_info_type_id} `
        }
        if (bind.name_rus) {
            sqlWhereClause += ` ${sqlWhereClause.trim() ? ` and ` : ` where `}  t.name_rus = '${bind.name_rus}' `
        }
        if (bind.name_kaz) {
            sqlWhereClause += ` ${sqlWhereClause.trim() ? ` and ` : ` where `}  t.name_kaz = '${bind.name_kaz}' `
        }
        if (bind.email_server_id) {
            sqlWhereClause += ` ${sqlWhereClause.trim() ? ` and ` : ` where `}  t.email_server_id = '${bind.email_server_id}' `
        }
        if (bind.name) {
