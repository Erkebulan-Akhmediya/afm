...
export async function putTestStanDB(client: Client, bind: any) {
    try {      
        console.log(bind)     
        const {rows: {[0]: data}} = await client.query({
            text: `
                update hr.test_competency_result 
                set stans = $1
                where test_session_id = $2;`,
            values: [
                bind.stans, 
                bind.test_session_id,
            ]
        }).catch((e: any) => { 
            throw `Ошибка hr.test_stans put db => ${e} ${JSON.stringify(bind)}`
        });
...
export async function test_competency_putDB(client: Client, bind: any) {
    try {           
        await client.query({
            ...
            throw `Ошибка hr.test_competency_putDB put db => ${e} ${JSON.stringify(bind)}`
        });
