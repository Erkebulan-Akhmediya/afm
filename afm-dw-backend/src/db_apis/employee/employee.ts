...
export async function employee_create(bind: any) {
  ...
  try {
    ...
    let sql_employee_insert :any = `INSERT INTO hr.employee (last_name_rus, first_name_rus, middle_name_rus, identification_number, birth_date, secret_name, apply_target, military_duty, nationality, phone_number, birthplace, address, citizen, other_information, gender_id)`
    let sql_employee_insert_values :any = `VALUES ('${bind.last_name}', '${bind.first_name}', '${bind.middle_name}', '${bind.identification_number}', '${bind.birth_date}', '${secret_name}', '${bind.apply_target}', ${bind.military_duty},'${bind.nationality}', '${bind.phone_number}', '${bind.birthplace}', '${bind.address}', '${bind.citizen}', '${bind.other_information}', ${bind.gender_id})`
    ...
    let sql_employee_language_insert :any =  'INSERT INTO hr.employee_language_new (employee_id, language, knowledge_id)'
    let sql_employee_language_insert_values :any = `VALUES`

    let sql_employee_sport_insert :any =  'INSERT INTO hr.employee_sport (employee_id, sportname, level_id)'
    ...
    if(bind.languages && bind.languages.length > 0){
      ...
      for(const language of bind.languages){
        sql_employee_language_insert_values += `(employee_id, '${language.language}', '${language.knowledge_id}')`; 
    ...
    if(bind.sports && bind.sports.length > 0){
      ...
      for(const sport of bind.sports){
        sql_employee_sport_insert_values += `(employee_id, '${sport.sportname}', '${sport.level_id}')`; 
    ...
    if(bind.crimes && bind.crimes.length > 0){
      ...
      for(const crime of bind.crimes){
        sql_employee_crimes_insert_values += `(employee_id, '${crime.crime_id}', '${crime.crime_status}', '${crime.employee_comment}')`; 
    ...
    BEGIN
      ...
      RETURNING id INTO employee_id;
        ...
        -- Insert into the Employee Family table by using the foreign key
        INSERT INTO hr.employee_family (employee_id, is_married, children_count)  
        VALUES (employee_id, ${bind.is_married}, ${(bind.children_count) ? bind.children_count : 0});
        ...
        INSERT INTO hr.employee_sur_rel (employee_id, sur_id, status)
        VALUES
        (employee_id, 1, ${bind.shortGapYear}),
        (employee_id, 2, ${bind.highGPA}),
        (employee_id, 3, ${bind.noCrimeExperience}),
        (employee_id, 4, ${bind.haveCertifications}),
        (employee_id, 5, ${bind.noBadWorkExperience}),
        (employee_id, 6, ${bind.noAgeLimit}),
        (employee_id, 7, ${bind.womanWithFewChildren}),
        (employee_id, 8, ${bind.haveSportAchievements}),
        (employee_id, 9, ${bind.noGovExperience});
    ...
    `;
    const employee: any = await client.query(query);

    return employee[1].rows[0];
  } catch (err) {
    console.log("______________Error in sur________________");
    log.error(err);
    throw err;
  } finally {
    if (client) {
...
export async function employee_get_db_new(
  ...
  let client;
  try {
    console.log('employee_get_db_new')
    client = get_client();
    await client.connect();
    ...
    let employee = await unviversal_query(
      `select 
              ...
              left join hr.employee_family ef on ef.employee_id = e.id
              left join ref.sport sport on sport.id = esp.level_id
              left join ref.language_knowledge lk on lk.id = el.knowledge_id
...
export async function candidate_get_db(
  ...
  try {
    ...
    log.error(`Error in candidate_get_db -> ${JSON.stringify(err)}`);
    throw err;
  } finally { 
    if (client) {
      await client.end();
...
export async function certified_candidate_get_db(
  bind: employee_get_binds
): Promise<Employee[]> {
  let client;
  try {
    console.log('certified_candidate_get_db');
    client = get_client();
    await client.connect();
...
`;
    ...
    log.error(`Error in candidates_get_all_db -> ${JSON.stringify(err)}`);
    throw err;
  } finally {
    if (client) {
      await client.end();
