...
export async function user_create (bind: any) {
    ...
    try {
        ...
        await client.query(`
        insert into hr.user
            (id, username, password, auth_type_id, create_user, update_user, language_id)
        values 
            (${bind.employeeId}, '${bind.login.trim().toUpperCase()}', '${bind.password}', 1, '${bind.username}', '${bind.username}', 1)
        `)
        return true;
    } catch (err) {
        log.error(err)
