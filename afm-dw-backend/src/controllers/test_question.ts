...
import countResult from '../utils/stans';
    ...
    try {
        ...
        if(isEssay){
            ...
            for (let i = 0; i < bind.testsessionanswer.length; i++) {
                ...
                answers.push((bind.testsessionanswer[i].userAnswerId - testSessionAnswer.correct_answer_id) + 1);   // from a to 1 b to 2 c to 3
                testSessionAnswer.correct_answer_id - bind.testsessionanswer[i].userAnswerId
        ...
        if(bind.testType != 3){
            ...
            if (bind.testsessionanswer[0].testType == 1) {
                let result: string = countResult(answers);
                console.log('goes here')
                await putTestStanDB(client, { stans: result, test_session_id: parseInt(bind.testsessionid,10) });
