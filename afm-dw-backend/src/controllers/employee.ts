...
import {employee_put_db_new,employee_get_db,employee_get_db_new,employee_education_db,employee_sports_db ,employee_fines_db,employee_adgs_db,employee_put_db,employee_languages_db ,employee_subordinates_get_db,employee_worklist_db, organization_subordinates_get_db, employee_get_positions_db, employee_get_access_scud_department_list, get_organization_dep_list, employee_birthday_get_db, existEmployeeExplanatory, candidates_get_all_approved_db, candidates_get_all_registered_db, employee_change_role_db, candidate_get_db, candidates_get_all_db, employees_get_all_db} from '../db_apis/employee/employee';
...
export async function candidate_get (req: express.Request, res: express.Response, next: express.NextFunction) {
    try {
        const bind: any = createBind(req);
        let employee
        if(Number(bind.role_id) == 83  || Number(bind.role_id) == 78  || Number(bind.role_id) == 79 || Number(bind.id)===Number(bind.useer_id)){
            employee = await candidate_get_db(bind);
        }else{
            return res.status(401).json({message: 'Пользователь не имеет доступа'})
        }
                res.locals.data = {
            statusCode: 200,
            data: employee
...
export async function employee_get_new (req: express.Request, res: express.Response, next: express.NextFunction) {
    try {
        const bind: any = createBind(req);
        let employee
        if (Number(bind.role_id) == 83 || Number(bind.id)===Number(bind.user_id)) {
            employee = await employee_get_db_new(bind);
        } else {
            return res.status(401).json({message: 'Пользователь не имеет доступа'})
        }
...
export async function candidates_get_all (req: express.Request, res: express.Response, next: express.NextFunction) {
    try {
        const bind: any = createBind(req);
        let employees
        if(Number(bind.role_id) == 83 || Number(bind.role_id) == 78 ||  Number(bind.role_id) == 79){
            employees = await candidates_get_all_db(bind);
        }else{
            return res.status(401).json({message: 'Пользователь не имеет доступа'})
        }
            res.locals.data = {
...
export async function employees_get_all (req: express.Request, res: express.Response, next: express.NextFunction) {
    try {
        const bind: any = createBind(req);
        let employees
        if(Number(bind.role_id) == 83){
            employees = await employees_get_all_db(bind);
        }else{
            return res.status(401).json({message: 'Пользователь не имеет доступа'})
        }
            res.locals.data = {
...
export async function employee_get_registered (req: express.Request, res: express.Response, next: express.NextFunction) {
    try {
        const bind: any = createBind(req);
        const employee = await employee_get_db_new(bind);
...
export async function employee_change_role (req: express.Request, res: express.Response, next: express.NextFunction) {
    let client: Client | null = null
    try {
        client = get_client(); await client.connect();
        const bind: any = createBind(req);
 
        if(bind.user_id == Number(bind.id)){
            return res.status(400).json({message: 'Невозможно изменить собственную роль'})
        }else if(Number(bind.role_id) == 83){
            await employee_change_role_db(client, {employee_id: req.params.id, role_id: req.body.role_id})
        }else{
            return res.status(401).json({message: 'Пользователь не имеет доступа'})
        }
        ...
        res.status(200).json({message: 'Роль пользователя успешно изменено'}).end();
        //res.status(204).end();
    } catch (error) {
        if (client) {
...
export async function employee_get_all (req: express.Request, res: express.Response, next: express.NextFunction) {
    try {
        ...
        let employees
        if(Number(bind.role_id) == 83){
            employees = await candidates_get_all_approved_db(bind);

        }else if(Number(bind.role_id) == 78 || Number(bind.role_id) == 79){
            employees = await candidates_get_all_registered_db(bind);
...
export async function employee_change_status (req: express.Request, res: express.Response, next: express.NextFunction) {
    let client: Client | null = null
    try {
        client = get_client(); await client.connect();
        ...
        await employee_put_db_new(client, {id: req.params.id, status_id: req.body.status_id, reason: req.body.reason})
