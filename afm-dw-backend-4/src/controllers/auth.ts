...
export async function userRegister (req: express.Request, res: express.Response, next: express.NextFunction){
    ...
    try {
        ...
        //Sur Validation
        let noBadWorkExperience = true
        let noCrimeExperience = true
        let noAgeLimit = true 
        let haveCertifications = false
        let highGPA = true
        let shortGapYear = false
        let womanWithFewChildren = true
        let haveSportAchievements = false
        let noGovExperience = true

        if(bind.sports){
            for(const sport of bind.sports){
                if(sport.level_id > 3 && sport.level_id < 7){
                    haveSportAchievements = true
                }
            }
        }
        if(bind.children_count){
            if(bind.gender_id == 2 && bind.children_count > 3){
                womanWithFewChildren = false
            }
        }
        if(bind.educations[0].date_to !== undefined){
            let graduatedDate = new Date(bind.educations[0].date_to)
            let startWorkDate 
            for(const work of bind.worklist){
                startWorkDate = new Date(work.date_from)
                if(startWorkDate >= graduatedDate){
                    shortGapYear = true
                }
            }
        ...
        if( (gpa/bind.educations.length) < 3.0){
            highGPA = false
        }

        for(const crime of bind.crimes){
            if(crime.crime_status){
                noCrimeExperience = false
            }
            if(crime.crime_id == 6 && crime.crime_status === true){
                noBadWorkExperience = false
            }
            if(crime.crime_id == 7 && crime.crime_status === true){
                noGovExperience = false
            }
        }
        ...
        if(bind.certifications && bind.achievements){
            if(bind.certifications.length>=1){
                haveCertifications = true
            }
            if(bind.achievements.length>=1){
                haveCertifications = true
            }
        }
        ...
        const user :any =await employee_create({
            apply_target: bind.apply_target, 
            military_duty: bind.military_duty, 
            is_married: bind.is_married,
            last_name: bind.lastname,
            gender_id: bind.gender_id,
            nationality: bind.nationality,
            first_name:bind.firstname, 
            middle_name:bind.middlename, 
            identification_number:bind.identification_number,
            children_count: bind.children_count, 
            birth_date:bind.birth_date, 
            citizen: bind.citizen, 
            address: bind.address, 
            phone_number: bind.phone_number, 
            birthplace: bind.birthplace, 
            other_information: bind.other_information, 
            educations: bind.educations, 
            worklist: bind.worklist, 
            languages: bind.languages, 
            sports: bind.sports, 
            username: bind.username, 
            password: passwordHash, 
            adgs_test: bind.adgs_test, 
            crimes: bind.crimes, 
            certifications: bind.certifications, 
            achievements: bind.achievements,
            noBadWorkExperience: noBadWorkExperience,
            noCrimeExperience:noCrimeExperience,
            noAgeLimit:noAgeLimit,
            haveCertifications:haveCertifications,
            highGPA:highGPA,
            shortGapYear:shortGapYear, 
            womanWithFewChildren: womanWithFewChildren, 
            haveSportAchievements: haveSportAchievements, 
            noGovExperience: noGovExperience
        })
