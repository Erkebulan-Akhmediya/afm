...
export async function pdf_summary (req: express.Request, res: express.Response, next: express.NextFunction) { 
    try {
        ...
        let query_return_data = await unviversal_query(
            ...
            jsonb_agg(DISTINCT jsonb_build_object(
                'grade', tcr.grade,
                'report_text', tcr.report_text,
                'test_id', tcr.test_id
            )) AS competency_results,
            (SELECT 
                test_question_qty 
             FROM hr.test_session 
             WHERE employee_id = $1
             AND test_id = 32
             LIMIT 1) AS test_question_qty,
            (SELECT 
                test_question_correct_qty 
             FROM hr.test_session 
             WHERE employee_id = $1
             AND test_id = 32
             LIMIT 1) AS test_question_correct_qty
        FROM hr.employee e
        ...
        LEFT JOIN hr.test_competency_result tcr ON e.id = tcr.employee_id
        WHERE e.id = $1
        GROUP BY
            e.id,
        ...
        switch (bind.work_place_type_report_code) {
            case 'pdf_workplace_base': 
                ...
                result.data['iq_test'] = ((query_return_data.rows[0].test_question_correct_qty / query_return_data.rows[0].test_question_qty) * 100).toFixed(2) + '%';

                console.log(query_return_data.rows[0]
                    .competency_results)
                result.data['essay_test'] = query_return_data.rows[0]
                    .competency_results.filter(obj => obj.test_id == 31)[0]['grade'] || 'Не пройден' 
                     
                result.data['psycho_test'] = query_return_data.rows[0]
                    .competency_results.filter(obj => obj.test_id == 30)[0]['grade'] || 'Не пройден'
                ...
                //result.data['educations'] = query_return_data.rows[0].educations[0].institute + '. (GPA ' + query_return_data.rows[0].educations[0].gpi + ').' 
        ...
        //     '72605697319-u0md72a0l0h7clfoj7225b3q1400alcj.apps.googleusercontent.com',
        //     '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',
        ...
        // let sprafka: string;
        // try {
        //     sprafka = await retGeneratedPDFInBase64(
        //         templateName, 
        //         bindFile,
        //     );
        //     console.log(sprafka);
        // } catch(err) {
        //     console.log('error in pdf_summary', err); 
        // }

        //http://localhost:8000/api/1.0/certified/candidate/193
        const websiteUrl = 'https://www.youtube.com/watch?v=dQw4w9WgXcQ';
        QRCode.toFile(
        ...
        minioClient.getObject('system', `afm_gov_qr.jpg`, async function(err: any, stream: any) {
            ...
            stream.on('end', async function() {
                ...
                let qrImage = fs.readFileSync(path.join(__dirname, 'qrcode.png'));
                const base64Image = qrImage.toString('base64');

                let bindFile = Object.assign({}, result.data)
                bindFile.src = "data:image/jpeg;base64," + base64Image

                try {
                    var returnData: any = await retGeneratedPDFInBase64(
                        templateName, 
                        bindFile,
                    );
...
export async function pdf_workplace (req: express.Request, res: express.Response, next: express.NextFunction) { 
    try {
        ...
        QRCode.toFile(path.join(__dirname, 'qrcode.png'), websiteUrl, (err)=>{
            if (err) throw err;
            console.log('QR code generated successfully!');
        });
