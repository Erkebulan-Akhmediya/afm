type HashMap<T extends number | string, M> = {
    [details in T]: M;
};

function countResult(arr: number[]): HashMap<string, number> {
    ...
    for (let num of arr) {
        ...
        if (num == 2) answers.push('b');
        if (num == 3) answers.push('c');
    }
...
const input = [3, 1, 2, 1, 3, 2, 3, 1, 2, 2];
