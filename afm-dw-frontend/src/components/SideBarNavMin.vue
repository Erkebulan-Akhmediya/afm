<template>
  <v-navigation-drawer
    ...
    :style="`
      ...
      (configCode == `afm` ? `background-color: #18073A;` : `background-color: #0e304c;` )"
    :mini-variant="mini"
  >
    <v-list-item class="px-5">
      ...
      >
        <v-img 
          class="logo" 
          src="../assets/img/logo_kz_white.png"
        ></v-img>
      </v-list-item-avatar>
      ...
      <div 
        class="d-flex flex-column justify-space-between sidebar-wrapper" 
        style="min-height: 570px;"
      >

      <v-list dense>
        <router-link 
          :to="`/employees/` + $crypto(String(userId))" 
          v-bind:class="{ routerLinkDisabled: isExternalPerformer }"
        >
...
<script>
  ...
  export default {
    ...
    created() {
      console.log(sessionStorage.getItem('role'), sessionStorage.getItem('roleName'));
      ...
      if (this.isExternalPerformer) {
        ...
        if (sessionStorage.getItem('role') == 77) {
          this.items.splice(0, 1);
          this.items.splice(0, 1);
          this.items.splice(0, 1);
        }
        if (sessionStorage.getItem('role') == 78) {
          this.items.splice(0, 1);
          this.items.splice(0, 1);
          this.items.splice(1, 1);
        }
        if (sessionStorage.getItem('role') == 79) {
          this.items.splice(0, 1);
          this.items.splice(0, 1);
        }
        if (sessionStorage.getItem('role') == 80) {
          this.items.splice(1, 1);
          this.items.splice(1, 1);
          this.items.splice(1, 1);
        }
        if (sessionStorage.getItem('role') == 81) {
          this.items.splice(0, 1);
          this.items.splice(1, 1);
          this.items.splice(1, 1);
        }
