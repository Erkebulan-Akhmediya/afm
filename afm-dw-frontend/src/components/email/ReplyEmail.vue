    ...
    <v-dialog v-model="dialog" max-width="1200px">
      <template v-slot:activator="{ on, attrs }">
        <v-list-item v-bind="attrs" v-on="on">
          ...
          <v-list-item-title class="mt-2">Ответить</v-list-item-title>
        </v-list-item>
      </template>
      <v-card>
        <v-card-title>
          <span color="black" class="text-h5">Новое сообщение-ответ</span>
        </v-card-title>
        <v-card-text>
          <v-container>
  ...
  export default {
    ...
    watch: {
      dialog() {
        ...
        this.emailSendAddressList.push(this.MESSAGE.from_email_address)
        this.subject = "Re:  " + this.MESSAGE.subject;

        this.textAsHtml =
          "<br><p>-------- Reply message --------</p>"+
