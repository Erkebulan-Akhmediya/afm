  ...
  >
    <template v-slot:[`item.avatar`]="{ item }">
      <v-avatar color="#fff" class="employees_table-avatar">
        ...
        <div v-if="item && item.is_subordinate && !item.is_disabled_pacs && !item.acsdata" style="position: relative; left: -8px; top: 6px; width: 0px;">
              <v-progress-circular
              indeterminate
              size="10"
              width="2"
...
import checkEmployeeDataAccess from "@/utils/check_employee_data_access";
  ...
  export default {
    ...
    methods: {
      ...
      async getLates(employees) {
        if(employees.length) {
          for(let i = 0; i < employees.length; i ++) {
            if((employees[i].is_subordinate && employees[i].is_disabled_pacs == false) || employees[i].id == this.$userData.id) {
              let data = await this.axios
              .get(`/api/1.0/employee/1/latecomes`, {
                params: {
                  type_code: 'employee_list',
                  employee_id: employees[i].id,
                  timeZ: -new Date().getTimezoneOffset()
                },
              })

                            this.$set(employees[i], `acsdata`, data.data[0].acsData[0])
            } 
          }
        }
      },
        ...
        .then(data => {
            if(data) {
              ...
              if(this.tableData.length) {
                this.getLates(this.tableData)
                setTimeout(() => {
                  this.goto(`scrollEmp${this.$store.state.lastEmployee}`)
                }, 0)
              }
            }
        }).finally (() => {
