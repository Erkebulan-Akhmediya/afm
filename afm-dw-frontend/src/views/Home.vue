...
export default {
  ...
  created() {
    Promise.all([
      ...
      .then((news) => {
        this.allNews = news
        if(this.allNews.length < this.newsParams.limit) {
          this.newsParams.isVisibleMoreBtn = false
        }

        this.allNews.map(item => item.image_link = `temp/randomnews/${this.getRandom()}.jpg`)
      })
      .catch((err) => {
          this.newsParams.isVisibleMoreBtn = false
          console.log('Ошибка получения новостей: ' + err)
      }),

      this.getEvents(2)
      .then((events) => {
        this.allEvents = events
        if(this.allEvents.length < this.eventParams.limit) {
          this.eventParams.isVisibleMoreBtn = false
        }
      })
      .catch((err) => {
          this.eventParams.isVisibleMoreBtn = false
          console.log('Ошибка получения объявлений: ' + err)
      }),

      this.getEvents(3)
      .then((assigns) => {

        this.allAssign = assigns

                if(this.allAssign.length < this.assignParams.limit) {
          this.assignParams.isVisibleMoreBtn = false
        }
      }).catch((err) => {
          this.assignParams.isVisibleMoreBtn = false
          console.log('Ошибка получения назначений: ' + err)
      }),
    ])
    .catch((err) => {
      alert(err)
