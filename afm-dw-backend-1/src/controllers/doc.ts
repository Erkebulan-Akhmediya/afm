...
let options: puppeteer.PDFOptions = {
    ...
    <body>
      <div class="content">
          <span>Сформировано в АИС "Цифровое рабочее место"</span>
          <span style="float: right;">Дата: `+moment(new Date()).format("DD.MM.YYYY HH:mm")+`</span>
      </div>
    </body>
  </html>
    `,
    ...
    try {
        ...
        switch (bind.work_place_type_report_code) {
            case 'pdf_workplace_base': 
                ...
                //result.data['educations'] = declension.parse(query_return_data.rows[0].educations).getForm(Case.Dat, false)
