    ...
    try {
        ...
        if(bind.crimes){    
            for(const crime of bind.crimes){
                await employee_crime_create({
                    employee_id:employee[0].id, 
                    crime_id: crime.crime_id,
                    crime_status: crime.crime_status, 
                    employee_comment: crime.employee_comment
                })
                    .catch(e => { 
                        throw `ошибка внедрения криминального прошлего : ${e.message}` 
                    })
            }
        }
